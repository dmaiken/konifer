...
Konifer is a high-performance, non-blocking REST API for managing assets such as images and media files.
...
Konifer's API design is inspired from the idea that the storing of assets is inherently hierarchical, much like a
directory structure. This pairs nicely with a REST API where resources are accessed by their path and "own" resources
defined underneath them. Konifer doesn't try to enforce any particular structure for assets, but it does provide a
...
What if the user has an album? Easy. Konifer provides a solution for that. Let's say your user went on a ski trip:
...
What if I don't have users? No problem! Konifer is incredibly generic and adaptable to your specific use case. Let's take
a
...
If Asset storage was all Konifer did, you would use AWS S3 or even a SAN. Konifer is more powerful though. In addition to
asset storage,
Konifer provides:

- asset processing (image resizing, filetype conversion)
- returning asset content directly from Konifer or returning a redirect to the backing object store
